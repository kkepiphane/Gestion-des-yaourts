...
class ModelIngrediant
{
    ...
    public function updateIngrediant($dIng, $nomIng, $prixUniatre, $quantite)
    {
        try {
            $db = dbConnect();
            $query = $db->prepare("UPDATE ingrediants SET nom_ing = ?, prixUnitaire = ?, quantite_dispo =? WHERE id_ing = '" . $dIng . "'");
